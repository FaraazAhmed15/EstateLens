"use client";

import { useState } from "react";
import Link from "next/link";
import {
  FaMapMarkerAlt,
  FaBed,
  FaRulerCombined,
  FaCar,
  FaShieldAlt,
  FaLeaf,
  FaArrowLeft,
  FaHome,
  FaPaperPlane,
  FaEraser,
  FaPhoneAlt,
  FaBuilding,
  FaTree,
  FaWater,
  FaDumbbell,
  FaWifi,
  FaSchool,
  FaHospital,
  FaShoppingBag,
  FaTrain,
} from "react-icons/fa";

export default function IndependentVilla() {
  const [message, setMessage] = useState("");

  return (
    <section className="max-w-6xl mx-auto px-6 mt-16 mb-20">

      {/* PROPERTY HEADER */}
      <div className="bg-[#FDF4E2] border border-[#B8A47C] rounded-xl p-10 shadow-md">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">

          {/* LEFT: IMAGE */}
          <div>
            <img
              src="/images/villa1.jpeg"
              alt="Independent Villa"
              className="w-full h-[360px] object-cover rounded-xl shadow-md"
            />
          </div>

          {/* RIGHT: DETAILS */}
          <div>
            <h2
              className="text-3xl font-bold text-[#6D1B1C] mb-2"
              style={{ fontFamily: "'Sanchez', serif" }}
            >
              Luxury Independent Villa
            </h2>

            <p className="text-[#1F3A2E] mb-2 flex items-center gap-2">
              <FaMapMarkerAlt /> Sarjapur Road, Bengaluru
            </p>

            <p className="text-2xl font-semibold text-[#6D1B1C] mb-4">
              ₹2.6 Crore
            </p>

            {/* PROPERTY HIGHLIGHTS */}
            <div className="grid grid-cols-2 gap-4 text-[#1F3A2E] mb-6">
              <p className="flex items-center gap-2">
                <FaBed /> 4 BHK Villa
              </p>
              <p className="flex items-center gap-2">
                <FaTree /> Private Garden
              </p>
              <p className="flex items-center gap-2">
                <FaRulerCombined /> 3,200 sq.ft Built-up
              </p>
              <p className="flex items-center gap-2">
                <FaCar /> 2 Covered Parking
              </p>
              <p className="flex items-center gap-2">
                <FaShieldAlt /> 24/7 Security
              </p>
              <p className="flex items-center gap-2">
                <FaBuilding /> Gated Community
              </p>
            </div>

            {/* ACTION BUTTONS */}
            <div className="mt-6 space-y-4">
              <button className="flex items-center gap-2 bg-[#6D1B1C] text-white px-6 py-2 rounded-md hover:bg-[#541516] transition">
                <FaPhoneAlt /> Request Owner Number
              </button>

              <div className="flex gap-4">
                <button
                  onClick={() => window.history.back()}
                  className="flex items-center gap-2 bg-white border border-[#B8A47C] text-[#6D1B1C]
                             px-5 py-2 rounded-md hover:bg-[#F4E9D8] transition"
                >
                  <FaArrowLeft /> Go Back
                </button>

                <Link
                  href="/"
                  className="flex items-center gap-2 bg-white border border-[#B8A47C] text-[#6D1B1C]
                             px-5 py-2 rounded-md hover:bg-[#F4E9D8] transition"
                >
                  <FaHome /> Home
                </Link>
              </div>
            </div>
          </div>
        </div>
      </div>

      {/* PROPERTY SPECIFICATIONS */}
      <div className="mt-12 bg-white border border-[#B8A47C] rounded-xl p-10 shadow-sm">
        <h3 className="text-2xl font-bold text-[#6D1B1C] mb-4">
          Property Specifications
        </h3>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-[#1F3A2E]">
          <p><strong>Property Type:</strong> Independent Villa</p>
          <p><strong>Configuration:</strong> 4 BHK</p>
          <p><strong>Built-up Area:</strong> 3,200 sq.ft</p>
          <p><strong>Land Area:</strong> 2,400 sq.ft</p>
          <p><strong>Possession:</strong> Ready to Move</p>
          <p><strong>Furnishing:</strong> Semi-Furnished</p>
          <p><strong>Parking:</strong> 2 Covered</p>
          <p><strong>Facing:</strong> East</p>
        </div>
      </div>

      {/* AMENITIES */}
      <div className="mt-12 bg-[#FDF4E2] border border-[#B8A47C] rounded-xl p-10 shadow-md">
        <h3 className="text-2xl font-bold text-[#6D1B1C] mb-4">
          Amenities
        </h3>

        <div className="grid grid-cols-2 md:grid-cols-3 gap-6 text-[#1F3A2E]">
          <p className="flex items-center gap-2"><FaWater /> Swimming Pool</p>
          <p className="flex items-center gap-2"><FaDumbbell /> Gym</p>
          <p className="flex items-center gap-2"><FaWifi /> High-Speed Internet</p>
          <p className="flex items-center gap-2"><FaShieldAlt /> 24/7 Security</p>
          <p className="flex items-center gap-2"><FaTree /> Landscaped Garden</p>
          <p className="flex items-center gap-2"><FaCar /> Dedicated Parking</p>
        </div>
      </div>

      {/* NEARBY LANDMARKS */}
      <div className="mt-12 bg-white border border-[#B8A47C] rounded-xl p-10 shadow-sm">
        <h3 className="text-2xl font-bold text-[#6D1B1C] mb-4">
          Nearby Landmarks
        </h3>

        <div className="grid grid-cols-2 md:grid-cols-3 gap-6 text-[#1F3A2E]">
          <p className="flex items-center gap-2"><FaSchool /> Oakridge School – 2 km</p>
          <p className="flex items-center gap-2"><FaHospital /> Manipal Hospital – 3 km</p>
          <p className="flex items-center gap-2"><FaShoppingBag /> Forum Mall – 4 km</p>
          <p className="flex items-center gap-2"><FaTrain /> Metro Station – 5 km</p>
        </div>
      </div>

      {/* WHY BUY THIS VILLA */}
      <div className="mt-12 bg-[#FDF4E2] border border-[#B8A47C] rounded-xl p-10 shadow-md">
        <h3 className="text-2xl font-bold text-[#6D1B1C] mb-4">
          Why Buy This Villa?
        </h3>

        <ul className="list-disc pl-5 text-[#1F3A2E] space-y-2">
          <li>Spacious interiors with premium fittings</li>
          <li>Private garden for family leisure</li>
          <li>Located in a high-growth real estate zone</li>
          <li>Close to IT parks and schools</li>
          <li>Secure gated community living</li>
        </ul>
      </div>

      {/* MESSAGE TO OWNER */}
      <div className="mt-12 bg-[#FDF4E2] border border-[#B8A47C] rounded-xl p-10 shadow-md">
        <h3 className="text-2xl font-bold text-[#6D1B1C] mb-4 text-center">
          Message to Owner
        </h3>

        <textarea
          rows={4}
          placeholder="Write your message to the property owner..."
          className="w-full border border-[#B8A47C] rounded-md px-4 py-2 focus:outline-none"
          value={message}
          onChange={(e) => setMessage(e.target.value)}
        />

        <div className="flex gap-4 mt-4">
          <button
            className="flex items-center gap-2 bg-[#6D1B1C] text-white px-8 py-2 rounded-md hover:bg-[#541516] transition"
          >
            <FaPaperPlane /> Send Message
          </button>

          <button
            onClick={() => setMessage("")}
            className="flex items-center gap-2 bg-white border border-[#B8A47C] text-[#6D1B1C]
                       px-6 py-2 rounded-md hover:bg-[#F4E9D8] transition"
          >
            <FaEraser /> Clear Message
          </button>
        </div>
      </div>

    </section>
  );
}